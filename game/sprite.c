...
bool Sprite_IsAnimated(Sprite_t *sprite) {
  return sprite->anim.animate || sprite->alive>=0;
}

bool Sprite_HasMovingSpriteInList(Sprite_t *list) {
  while(list!=NULL) {
    if (Sprite_IsMoving(list)) { /* found a moving sprite */
      return true;
    }
    list = list->next; /* continue searching */
  }
  return false;
}
...
bool Sprite_HasAnimatedSpriteInList(Sprite_t *list) {
  while(list!=NULL) {
    if (Sprite_IsAnimated(list)) { /* found a animated sprite */
      return true;
    }
    list = list->next; /* continue searching */
  }
