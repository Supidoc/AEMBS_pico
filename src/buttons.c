...
void Buttons_Init(void)
{
    ...
    PL_CONFIG_BOARD_ID_TINYK22_APROG_HAT_V7 /* V7 does not have hardware pull-ups on the board */
    ...
    PORT_SetPinInterruptConfig(BUTTONS_PINS_NAVLEFT_PORT, BUTTONS_PINS_NAVLEFT_PIN,
                               kPORT_InterruptFallingEdge);
    PORT_SetPinInterruptConfig(BUTTONS_PINS_NAVRIGHT_PORT, BUTTONS_PINS_NAVRIGHT_PIN,
                               kPORT_InterruptFallingEdge);
    NVIC_SetPriority(PORTA_IRQn, configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY);
    EnableIRQ(PORTA_IRQn);

    PORT_SetPinInterruptConfig(BUTTONS_PINS_NAVDOWN_PORT, BUTTONS_PINS_NAVDOWN_PIN,
                               kPORT_InterruptFallingEdge);
    PORT_SetPinInterruptConfig(BUTTONS_PINS_NAVUP_PORT, BUTTONS_PINS_NAVUP_PIN,
                               kPORT_InterruptFallingEdge);
    PORT_SetPinInterruptConfig(BUTTONS_PINS_NAVCENTER_PORT, BUTTONS_PINS_NAVCENTER_PIN,
                               kPORT_InterruptFallingEdge);
    ...
    gpio_set_irq_enabled_with_callback(BUTTONS_PINS_NAVCENTER_PIN, GPIO_IRQ_EDGE_FALL, true,
                                       &gpio_IsrCallback);
    gpio_set_irq_enabled_with_callback(BUTTONS_PINS_NAVUP_PIN, GPIO_IRQ_EDGE_FALL, true,
                                       &gpio_IsrCallback);
    gpio_set_irq_enabled_with_callback(BUTTONS_PINS_NAVDOWN_PIN, GPIO_IRQ_EDGE_FALL, true,
                                       &gpio_IsrCallback);
    gpio_set_irq_enabled_with_callback(BUTTONS_PINS_NAVLEFT_PIN, GPIO_IRQ_EDGE_FALL, true,
                                       &gpio_IsrCallback);
    gpio_set_irq_enabled_with_callback(BUTTONS_PINS_NAVRIGHT_PIN, GPIO_IRQ_EDGE_FALL, true,
                                       &gpio_IsrCallback);
#endif
}
