...
#endif
#if LEDS_CONFIG_HAS_GREEN_PICO_LED
        case LEDS_PICO_GREEN:
            McuLED_On(ledPicoGreen);
            break;
#endif
        default:
            break; /* error */
...
#if LEDS_CONFIG_HAS_BLUE_TINY_LED
        case LEDS_TINY_BLUE:
            McuLED_Toggle(ledTiny);
            break;
#endif
#if LEDS_CONFIG_HAS_GREEN_PICO_LED
        case LEDS_PICO_GREEN:
            McuLED_Toggle(ledPicoGreen);
            break;
#endif
        default:
            break; /* error */
...
static uint8_t PrintStatus(McuShell_ConstStdIOType* io)
{
    ...
    McuShell_SendStatusStr((const unsigned char*)"led",
                           (const unsigned char*)"LED module status\r\n", io->stdOut);
#if LEDS_CONFIG_HAS_RED_LED
    ...
    McuUtility_strcat(buf, sizeof(buf), "\r\n");
    McuShell_SendStatusStr((const unsigned char*)"  picogreen", (const unsigned char*)buf,
                           io->stdOut);
#endif
#if LEDS_CONFIG_HAS_BLUE_TINY_LED
    McuLED_GetLedStatusString(ledTiny, buf, sizeof(buf));
    McuUtility_strcat(buf, sizeof(buf), "\r\n");
    McuShell_SendStatusStr((const unsigned char*)"  tinyblue", (const unsigned char*)buf,
                           io->stdOut);
#endif
    return ERR_OK;
}
...
#if LEDS_CONFIG_HAS_RED_LED
    McuShell_SendHelpStr((unsigned char*)"  red on|off|neg",
                         (const unsigned char*)"Control red LED\r\n", io->stdOut);
#endif
#if LEDS_CONFIG_HAS_GREEN_LED
    McuShell_SendHelpStr((unsigned char*)"  green on|off|neg",
                         (const unsigned char*)"Control green LED\r\n", io->stdOut);
#endif
#if LEDS_CONFIG_HAS_BLUE_LED
    McuShell_SendHelpStr((unsigned char*)"  blue on|off|neg",
                         (const unsigned char*)"Control blue LED\r\n", io->stdOut);
#endif
#if LEDS_CONFIG_HAS_ORANGE_LED
    McuShell_SendHelpStr((unsigned char*)"  orange on|off|neg",
                         (const unsigned char*)"Control orange LED\r\n", io->stdOut);
#endif
#if LEDS_CONFIG_HAS_GREEN_PICO_LED
    McuShell_SendHelpStr((unsigned char*)"  picogreen on|off|neg",
                         (const unsigned char*)"Control pico green LED\r\n", io->stdOut);
#endif
#if LEDS_CONFIG_HAS_BLUE_TINY_LED
    McuShell_SendHelpStr((unsigned char*)"  tinyblue on|off|neg",
                         (const unsigned char*)"Control tiny blue LED\r\n", io->stdOut);
#endif
    return ERR_OK;
}
...
uint8_t Leds_ParseCommand(const uint8_t* cmd, bool* handled, McuShell_ConstStdIOType* io)
{
    if (McuUtility_strcmp((char*)cmd, McuShell_CMD_HELP) == 0 ||
        McuUtility_strcmp((char*)cmd, "led help") == 0)
    {
        *handled = TRUE;
        return PrintHelp(io);
    }
    else if ((McuUtility_strcmp((char*)cmd, McuShell_CMD_STATUS) == 0) ||
             (McuUtility_strcmp((char*)cmd, "led status") == 0))
    {
        *handled = TRUE;
...
void Leds_Init(void)
{
    ...
    LEDS_CONFIG_ENABLE_CLOCK(); /* enable clocking or initialize GPIO as required
                                   by hardware */
